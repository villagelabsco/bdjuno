...
func (fa DbReputationFeedback) FromProto(f *reputationtypes.Feedback) (DbReputationFeedback, error) {
	pos, err := json.Marshal(f.Positive)
	if err != nil {
		return DbReputationFeedback{}, fmt.Errorf("error marshalling positive: %v", err)
	}
	neg, err := json.Marshal(f.Negative)
	if err != nil {
		return DbReputationFeedback{}, fmt.Errorf("error marshalling negative: %v", err)
	}
	neu, err := json.Marshal(f.Neutral)
	if err != nil {
		return DbReputationFeedback{}, fmt.Errorf("error marshalling neutral: %v", err)
	}
	feedbackers, err := json.Marshal(f.Feedbackers)
	if err != nil {
		return DbReputationFeedback{}, fmt.Errorf("error marshalling feedbackers: %v", err)
	}
	...
	return DbReputationFeedback{
		...
		LastChange:  time.Unix(int64(f.LastChange), 0),
	}, nil
}

func (fa DbReputationFeedback) ToProto() (*reputationtypes.Feedback, error) {
	...
	return &reputationtypes.Feedback{
		Index:       fa.Index,
		LastChange:  uint64(fa.LastChange.Unix()),
